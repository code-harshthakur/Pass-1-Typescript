...
const motData: string = fs.readFileSync(
  path.join(__dirname, "../data/mot.json"),
  "utf-8",
);
...
function processLiteral(operand: string, context: Context): void {
  const literalValue: string = operand.slice(
    operand.indexOf("'") + 1,
    operand.length - 1,
  );
...
function pass1Assembler(inputFilePath: string, outputFilePath: string): void {
  ...
  console.log(
    "Pass 1 Assembler completed. Output, Symbol Tabel, and Literal Table written to:",
    outputFilePath,
  );
}
